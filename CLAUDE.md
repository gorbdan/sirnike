...
- Задачи между ролями — файлы в `docs/briefs/`, должностные инструкции — `docs/roles/`.
  ...
  ⛔ **Роль/агент никогда сам не вызывает другую роль через `Agent`-тул** (frontend
  не спавнит backend и наоборот) — обе сессии работают в одном рабочем дереве и
  начинают одновременно править одни файлы, отсюда конфликты. Задача не своей
  зоны → бриф в `docs/briefs/<роль>.md`, переключение роли — решение Ани.
