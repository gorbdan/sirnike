...
- **Админы** (`ADMIN_IDS`): платят изюминками как обычные юзеры (Аня заранее начисляет себе
  баланс — байпаса в коде фото/видео/аватара НЕТ и не нужно, бесплатна только студия);
  использование шаблонов и генерации не пишутся в статистику/P&L (`is_admin_test=1`).
...
- Маркетинговые материалы (планы, контент, отчёты) — в Notion-хабе «Сырник — Маркетинг»
  через Notion MCP, НЕ в git (правило Ани). В репо — только роли/брифы/спеки/аудиты/код.
